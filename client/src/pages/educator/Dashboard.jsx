...
const Dashboard = () => {
  ...
  const { userData, allCourses } = useContext(AppContext);

  // Check if user is educator
  const isEducator = userData?.isEducator;
  const educatorId = isEducator ? userData._id : null;

  // Fetch dashboard data (simulate)
  const fetchDashboardData = async () => {
    const data = await new Promise((resolve) =>
      setTimeout(() => resolve(dummyDashboardData), 1000)
    );
    setDashboardData(data);
  };

  // Calculate total enrollments for this educator
  const getTotalEnrollments = (allCourses, educatorId) => {
    if (!educatorId) return 0;
    return (
      allCourses?.reduce((total, course) => {
        if (course?.educator?._id === educatorId) {
  ...
  // Calculate total earnings for this educator
  const getTotalEarnings = (allCourses, educatorId) => {
    if (!educatorId) return 0;
    return (
      allCourses?.reduce((total, course) => {
        if (course?.educator?._id === educatorId) {
          ...
          return total + enrolledCount * coursePrice;
        }
        return total;
      }, 0) || 0
    );
  ...
  const totalEnrollments = getTotalEnrollments(allCourses, educatorId);
  const totalEarnings = getTotalEarnings(allCourses, educatorId);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-gray-600 text-lg font-medium">
          You are not an educator. Please login as an educator to access the
          dashboard.
        </p>
      </div>
    );
    ...
    <div className="min-h-screen flex flex-col items-start justify-between md:p-8 md:pt-0 p-4 pt-8 bg-gradient-to-br from-indigo-50/40 via-purple-50/30 to-pink-50/40">
      ...
      <div className="w-full mb-8 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white/80 backdrop-blur-md rounded-xl shadow-lg border border-white/20 p-6 text-center">
          ...
          <div className="text-3xl font-bold text-green-600 mb-2">
            {allCourses?.filter(
              (course) => course?.educator?._id === educatorId
            )?.length || 0}
          </div>
      ...
      <div className="w-full flex-1">
        <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-lg border border-white/20 overflow-hidden">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200/60">
                {dashboardData.enrolledStudentsData
                  .filter((item) => item.courseEducatorId === educatorId)
                  .map((item, index) => (
                    <tr
          ...
          {dashboardData.enrolledStudentsData.filter(
            (item) => item.courseEducatorId === educatorId
          ).length === 0 && (
